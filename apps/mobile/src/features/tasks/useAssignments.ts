...
export async function loadAssignmentsImpl(
  deps: LoadAssignmentsDeps,
  options?: { silent?: boolean }
) {
  const { session, supabaseClient, resetStore, setLoading, setRefreshing, setStoreError, setAssignments } =
    deps;
...
type UpdateAssignmentDeps = {
  assignments: Assignment[];
  supabaseClient: typeof supabase;
  updateAssignmentInStore: (assignmentId: string, updater: (assignment: Assignment) => Assignment) => void;
  enqueueJob: typeof enqueueAssignmentStatusJob;
};

export async function updateAssignmentStatusImpl(
  deps: UpdateAssignmentDeps,
  assignmentId: string,
  nextStatus: AssignmentStatus,
  options?: { completionNote?: string | null }
): Promise<boolean> {
  const { assignments, supabaseClient, updateAssignmentInStore, enqueueJob } = deps;
...
export function useAssignments(session: Session | null): UseAssignmentsResult {
  ...
  const loadAssignments = useCallback(
    ...
    [resetStore, session, setAssignments, setLoading, setRefreshing, setStoreError]
  );
  ...
  const updateAssignmentStatus = useCallback(
    (assignmentId: string, nextStatus: AssignmentStatus, options?: { completionNote?: string | null }) =>
      updateAssignmentStatusImpl(
        {
          assignments,
          supabaseClient: supabase,
          updateAssignmentInStore,
          enqueueJob: enqueueAssignmentStatusJob,
        },
        assignmentId,
        nextStatus,
        options
